...
public class ResultsServices 
{
	...
	private JSONObject buildQuestionResults(Long questionID, Long userID, boolean exportMode, boolean instructorMode, boolean studentReviewResults)
	{
		...
		if(questionInQuestion.getResultsPublished() || userID == null || instructorMode)
		{
			...
			int totalCorrectPointsPossible = 0; //for all question types
			int totalPointsEarnedForQuestion = 0; //for all question types
			...
			if(questionInQuestion.getType() == QuestionType.MULTIPLE_CHOICE )
			{
				...
				for(QuestionOption O : options)
				{
					...
					if(O.getIsCorrect())
					{
						totalCorrectPointsPossible += O.getPointValue();
					}
				}
				...
				for(com.turingpoint.dbmodel.db.mapping.Response R : responses)
				{
					...
					QuestionOption selectedQuestionOption = QuestionOption.getQuestionOption(selectedOption);
					...
					{
						totalPointsEarnedForQuestion = selectedQuestionOption.getPointValue();
						...
						userResponseAttributes.put("points", totalPointsEarnedForQuestion);
				...
				responseObject.put("value", totalCorrectPointsPossible);
			}
			else if (questionInQuestion.getType() == QuestionType.FILL_IN_BLANK || questionInQuestion.getType() == QuestionType.NUMERIC_RESPONSE )
			{
				...
				totalCorrectPointsPossible += option.getPointValue();
				...
				for(com.turingpoint.dbmodel.db.mapping.Response R : responses)
				{
						...
						QuestionOption optionSelectedByUser = QuestionOptionPersister.getQuestionOptions(questionID).get(0);
						...
						if(!studentReviewResults)
						{
							...
							if(correct)
							{
								totalPointsEarnedForQuestion += optionSelectedByUser.getPointValue();
							}
				...
				for(com.turingpoint.dbmodel.db.mapping.Response R : responses)
				{
					...
					for (String S : responsesBrokenUp)
					{
						...
						if(!studentReviewResults)
						{
							...
							userResponseAttributes.put("userID", whoAnswered);
							userResponseAttributes.put("date", dateOfResponse);
							userResponseAttributes.put("points", selectedQuestionOption.getPointValue());
							allUserResponses.getJSONArray(selectedOption+"").put(userResponseAttributes);
						}
